...
#include "mod_libfabric_connectionless.h"
...
HRT_TIMESTAMP_T benchmark_start_2_cl;

uint64_t get_timestamp_ticks_2_cl() {
  HRT_TIMESTAMP_T now;
...
double get_timestamp_usec_2_cl() {
  HRT_TIMESTAMP_T now;
  HRT_GET_TIMESTAMP(now);
  long long ticks;
  HRT_GET_ELAPSED_TICKS(benchmark_start_2_cl, now, &ticks);
  double now_usec = HRT_GET_USEC(ticks);
  return now_usec;
}

struct ng_module libfabric_cl_module = {
    .name = "libfabric_cl",
    .desc = "This mod uses libfabric for data transmission in connectionless mode.",
    ...
    .malloc = libfabric_cl_malloc,
    .getopt = libfabric_cl_getopt,
    .init = libfabric_cl_init,
    .shutdown = libfabric_cl_shutdown,
    .usage = libfabric_cl_usage,
    .writemanpage = libfabric_cl_writemanpage,
    .sendto = libfabric_cl_sendto,
    .recvfrom = libfabric_cl_recvfrom,
    .set_blocking = libfabric_cl_set_blocking,
    .isendto = libfabric_cl_isendto,
    .irecvfrom = libfabric_cl_irecvfrom,
    .test = libfabric_cl_test,
};
...
} libfabric_cl_network_t;

typedef struct {
    struct fid_ep *ep;
    struct fid_cq *cq;
    struct fid_eq *eq;
} libfabric_cl_connected_endpoint_t;

typedef struct {
    struct fid_pep *pep;
    struct fid_eq *eq;
    libfabric_cl_address_t addr;
} libfabric_cl_listening_endpoint_t;

typedef struct libfabric_cl_memory_region_info_t {
    ...
    struct libfabric_cl_memory_region_info_t *next; /* Pointer to the next memory region info */
} libfabric_cl_memory_region_info_t;
...
typedef struct {
    ...
    libfabric_cl_network_t network;
    libfabric_cl_connected_endpoint_t *peer_connections;
    libfabric_cl_memory_region_info_t *memory_regions; /* linked list of memory regions info */

    libfabric_cl_peer_info_t my_peer_info; /* info about this node to send to peers */
    size_t nodes_no;                 /* number of nodes in the benchmark */
    size_t my_node_id;               /* this node's id */
    libfabric_cl_peer_info_t *peer_info;   /* info about all nodes */


} libfabric_cl_private_data_t;

static libfabric_cl_private_data_t module_data;

// Returned by async operations as handle to check for completion
typedef struct {
    bool completed;
    struct fid_cq *cq;
} libfabric_cl_completion_handle_t;

#define MAX_HANDLES 1024

libfabric_cl_completion_handle_t* libfabric_cl_get_completion_handle() {
    static libfabric_cl_completion_handle_t handles[MAX_HANDLES];
...
static int libfabric_cl_getopt(int argc, char **argv, struct ng_options *global_opts) {
    ...
    VERBOSE_LOG("Parsing command-line options for libfabric_cl module\n");
    VERBOSE_LOG("argc = %d\n", argc);
...
libfabric_cl_network_t libfabric_cl_open_network(struct fi_info *fi) {
    ...
    libfabric_cl_network_t network = {0};
    network.fi = fi;
...
libfabric_cl_connected_endpoint_t libfabric_cl_connect_to_server(
    libfabric_cl_network_t *network, const libfabric_cl_address_t *server_address) {
    ...
    libfabric_cl_connected_endpoint_t connected_ep;
    connected_ep.ep = ep;
    connected_ep.cq = cq;
    connected_ep.eq = eq;
...
libfabric_cl_listening_endpoint_t libfabric_cl_create_listening_endpoint(
    libfabric_cl_network_t *network) {
    ...
    libfabric_cl_address_t listening_addr =
        libfabric_cl_address_from_bytes(addr, addrlen);
    ...
    libfabric_cl_listening_endpoint_t listening_ep;
...
void libfabric_cl_close_listening_endpoint(
    libfabric_cl_listening_endpoint_t *listening_ep) {
    if (listening_ep->pep) {
...
libfabric_cl_connected_endpoint_t libfabric_cl_accept_connection(
    libfabric_cl_network_t *network,
    libfabric_cl_listening_endpoint_t *listening_ep) {
...
struct fid_mr *libfabric_cl_register_memory_region(void *ptr, size_t size);

/* exchanges peer_info and sets nodes_no and my_node_id */
int MPI_cl_data_exchange(struct ng_options *global_opts,
                      libfabric_cl_private_data_t *module_data,
                      libfabric_cl_peer_info_t my_peer_info) {
    ...
    module_data->peer_info =
        malloc(sizeof(libfabric_cl_peer_info_t) * module_data->nodes_no);

    // Exchange peer_info using MPI
    MPI_Allgather(&my_peer_info, sizeof(libfabric_cl_peer_info_t), MPI_BYTE,
                  module_data->peer_info, sizeof(libfabric_cl_peer_info_t), MPI_BYTE,
                  MPI_COMM_WORLD);
}

static int libfabric_cl_init(struct ng_options *global_opts) {
    VERBOSE_LOG("Initializing libfabric_cl module\n");
    ...
    struct fi_info *info = libfabric_cl_get_info(module_data.provider_name);
    module_data.network = libfabric_cl_open_network(info);
    libfabric_cl_listening_endpoint_t listening_endpoint =
        libfabric_cl_create_listening_endpoint(&module_data.network);
    ...
    module_data.local_mr = libfabric_cl_register_memory_region(
    ...
    module_data.remote_mr = libfabric_cl_register_memory_region(
    ...
    MPI_cl_data_exchange(global_opts, &module_data, my_peer_info);

    // Initialize connections with each peer
    module_data.peer_connections =
        malloc(sizeof(libfabric_cl_connected_endpoint_t) * module_data.nodes_no);

    // Accept connections from peers with lower IDs
    for (size_t i = 0; i < module_data.my_node_id; i++) {
        libfabric_cl_connected_endpoint_t connected_endpoint =
            libfabric_cl_accept_connection(&module_data.network,
                                        &listening_endpoint);
    ...
    HRT_GET_TIMESTAMP(benchmark_start_2_cl);

    return 0;
}

void libfabric_cl_add_memory_region(
    void *start,
    size_t length, 
    struct fid_mr *mr
) {
    libfabric_cl_memory_region_info_t *new_info =
        malloc(sizeof(libfabric_cl_memory_region_info_t));
...
libfabric_cl_memory_region_info_t *libfabric_cl_get_memory_region_info(void *ptr) {
    VERBOSE_LOG("Looking up memory region for pointer %p\n", ptr);

    libfabric_cl_memory_region_info_t *current = module_data.memory_regions;
    while (current) {
...
void *libfabric_cl_malloc(size_t size) {
    VERBOSE_LOG("Allocating %zu bytes of memory\n", size);

    void *ptr = malloc(size);
    
    struct fid_mr *mr = libfabric_cl_register_memory_region(ptr, size);

    // Save the memory region info
    libfabric_cl_add_memory_region(ptr, size, mr);

    VERBOSE_LOG("Memory allocated at %p with size %zu\n", ptr, size);
    return ptr;
}

void wait_for_cl_completion(struct fid_cq *cq) {
    while (true) {
...
// to have a pointer to libfabric_cl_completion_handle_t in a op_context field,
// this pointer is used to mark the operation as completed.
void read_all_cl_completions(struct fid_cq *cq) {
    ...
    while (true) {
        ...
        if (ret > 0) {
            ...
            libfabric_cl_completion_handle_t *handle =
                (libfabric_cl_completion_handle_t*)comp.op_context;

            //Mark comp as completed
            if (handle != NULL) {
                handle->completed = true;
                double time_stamp = get_timestamp_usec_2_cl();
...
int libfabric_cl_set_blocking(int fd, int blocking) {
...
static int libfabric_cl_isendto(int dst, void *buffer, int size, NG_Request *req) {
    ...
    libfabric_cl_completion_handle_t *handle = libfabric_cl_get_completion_handle();
    ...
    *req = (NG_Request)handle;
    ...
    switch (module_data.rdma_operation) {
        case 's': // send (send/recv)
        {
            ...
            // void *desc = libfabric_cl_get_memory_region_info(buffer)->desc;
    ...
    *req = (NG_Request)handle;
    ...
    switch (module_data.rdma_operation) {
        case 's': // receive (send/recv)
        {
            ...
            // libfabric_cl_memory_region_info_t *mr_info = libfabric_cl_get_memory_region_info(buffer);
            ...
            // wait_for_cl_completion(module_data.peer_connections[src].cq);
            break;
        }
        case 'r': // RDMA read
...
static int libfabric_cl_test(NG_Request *req) {
    libfabric_cl_completion_handle_t *handle = (libfabric_cl_completion_handle_t *)(*req);

    // // Read all completions from the CQ
    read_all_cl_completions(handle->cq);
...
static int libfabric_cl_sendto(int dst, void *buffer, int size) {
    NG_Request req;
    libfabric_cl_isendto(dst, buffer, size, &req);
    
    // Wait for completion
    while (libfabric_cl_test(&req) != 0) {
        // Busy wait
    }
...
static int libfabric_cl_recvfrom(int src, void *buffer, int size) {
    NG_Request req;
    libfabric_cl_irecvfrom(src, buffer, size, &req);
...
/* module registration */
int register_libfabric_cl(void) {
   ng_register_module(&libfabric_cl_module);
   return 0;
}

#else

/* Don't register this module if NG_MOD_LIBFABRIC_CONNECTIONLESS is not defined */
int register_libfabric_cl(void) {
   return 0;
}
